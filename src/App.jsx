...
const App = () => {
  ...
  const botQuery = async (query) => {
    ...
    try {
      ...
      const botReply =
        typeof response.response.bot_response === "object"
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages([
        ...
        {
          text: "Couldn't connect to chatbot! Please try later.",
          sender: "bot",
          speaker: false,
        },
      ]);
  ...
  const homeFAQs = [
    {
      ...
      answer: t("wahmisocitdsasutlidtlaiiotbraot"),
    },
    {
      question: t("hdmseds"),
      answer: t("wpdstaml"),
  ...
  const helpFAQs = [
    ...
    {
      question: t(
        "What is the average turnaround time for resolving support tickets?"
      ),
      answer: t(
  ...
  return (
    <>
      <div
        className={`fixed z-10 max-w-[350px] overflow-hidden !ease-[cubic-bezier(0.77,0,0.175,1)] !duration-[400ms] flex flex-col items-center bottom-[20px] ${
          isChatbotOpen
            ...
            : "justify-center right-[20px] w-[60px] h-[60px] opacity-80 cursor-pointer hover:opacity-100 rounded-[50%] bg-[#058fd9]"
        }`}
      >
        {isChatbotOpen ? (
          <>
            {/* Header */}
            <div className="w-full bg-[#058fd9] shadow-md px-5 flex justify-between items-center shadow min-h-[60px]">
            ...
            <div className="grow relative flex flex-col px-[18px]">
              {/* FAQ Panel */}
              <div
                className={`noscroll !duration-600 z-30 absolute flex-col bg-white top-0 left-1/2 -translate-x-1/2 overflow-auto min-w-full ${
                  isFAQOpen ? "h-full grow flex" : "h-0 grow-0"
                }`}
              >
                  ...
                  {homeFAQs.map((FAQ, index) => (
                    <div key={"FAQ-" + index}>
                      ...
                      >
                        ...
                        <FaChevronDown
                          className={`${
                            openFAQIndex === "FAQ-" + index ? "rotate-180" : ""
                          } transition-transform`}
                        />
                      </div>
                      <div
                        className={`${
                          openFAQIndex === "FAQ-" + index
                            ? "max-h-fit py-[1rem] px-[1rem] border border-[#2E5BFF47] mb-3"
                            : "max-h-0 p-0 border-0"
                        } transition-all duration-300 overflow-hidden mt-2 items-center bg-[#058fd9] text-white shadow-xl text-[12px] rounded-[18px] cursor-pointer`}
                      >
                        <p>{FAQ.answer}</p>
                      </div>
                    </div>
              ...
              <div
                className={`noscroll !duration-600 z-30 absolute flex-col bg-white top-0 left-1/2 -translate-x-1/2 overflow-auto min-w-full ${
                  isHelpPageOpen ? "h-full grow flex" : "h-0 grow-0"
                }`}
              >
                <div className="flex items-center px-5 pt-5">
                  <IoIosArrowBack
              ...
              <div
                className={`${
                  isChatStarted || isFAQOpen || isHelpPageOpen
                    ? "opacity-0"
                    : "opacity-100"
                } z-10 !duration-[1s] delay-200 grow flex flex-col items-center justify-center`}
              >
                <p className="text-[17px] text-center font-semibold text-[#058fd9]">
                  {t("wtm")}
                </p>
              ...
              {/* Chat Body */}
              <div
                className={`!duration-[1s] z-20 absolute flex-col bg-white bottom-0 left-1/2 -translate-x-1/2 min-w-full ${
                  isChatStarted ? "h-full grow flex" : "h-0 grow-0"
                }`}
              >
                <div className="noscroll chat-body flex flex-col px-5 pt-4 overflow-auto">
                  {messages.map((message, index) => (
                    ...
                    >
                      <div
                        className={`px-3 flex animate__animated flex-col py-2 rounded-[20px] break-words text-sm ${
                          message.sender === "user"
            ...
            <div
              className={`${
                isFAQOpen || isHelpPageOpen || showUserForm
                  ? "!translate-y-[120px] !h-0"
                  : "!translate-y-0 h-[120px]"
              } !duration-[1s] overflow-hidden w-full ${
                isChatStarted ? "px-0 h-fit" : "px-[20px]"
              } flex flex-col items-center justify-center`}
            >
              <div
                className={`border w-full flex items-center justify-center py-[0.6rem] gap-3 px-[1rem] border-[#EBECED] ${
                  isChatStarted ? "rounded-t-[18px]" : "rounded-[18px]"
                }`}
              >
                {/* Mic */}
                <button
                  ...
                  className={`rounded-full flex justify-center items-center ${
                    isListening ? "bg-red-500" : "bg-[#058fd9]"
                ...
                {/* Send */}
                <button
                  className={`rounded-full flex justify-center items-center transition-transform duration-200 ${
                    input.trim().length === 0 ? "bg-[#F0F0F0]" : "bg-[#058fd9]"
              ...
              <div
                className={`${
                  isChatStarted
                    ? "translate-y-[100px] h-0"
                    : "translate-y-0 h-[30px] mt-[10px]"
